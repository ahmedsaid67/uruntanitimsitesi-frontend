...
import { Container, Typography, Paper,Pagination, Table, TableBody, Tooltip,TableCell, TableHead, TableRow, Button, Dialog, DialogTitle,Radio, RadioGroup,FormControl, FormLabel,DialogContent, TextField, Checkbox, FormControlLabel, DialogActions } from '@mui/material';
...
export default function GorselBasin() {
    ...
    const handlePageChange = (event, value) => {
      ...
      const handleSave = (editedItem) => {
        ...
        if (!editedItem.name || !editedItem.img || !editedItem.url || !editedItem.order || !editedItem.device) {
        ...
        formData.append("device", editedItem["device"]);
      ...
      const handleAddNewItem = () => {
        ...
        if (!newItem.name || !newItem.img || !newItem.url || !newItem.order || !newItem.device) {
        ...
        formData.append("device", newItem["device"]);
    ...
    const handleConfirmDelete = () => {
      ...
      axios.post(API_ROUTES.SLIDERS_DELETE, { ids: selectedIds })
        .then(() => {
          return axios.get(API_ROUTES.SLIDERS);
        })
        .then((response) => {
                  ...
                  <Table size="small">
                    <TableHead>
                      <TableRow style={{ backgroundColor: '#1976d2' }}> 
                        ...
                        <TableCell style={{ fontSize: '0.85rem', fontWeight: 'bold', color: '#fff' }}>Kategori</TableCell>
                    ...
                    <TableBody>
                      {data.map(row => (
                        <TableRow key={row.id}>
                          ...
                          <TableCell style={{ fontSize: '0.75rem' }}>{row.device}</TableCell>
        ...
        <DialogContent>
          ...
          <div style={{ marginTop: "20px", marginBottom: "20px", padding: "10px", border: "1px solid #ccc", borderRadius: "5px" }}>
              <FormControl component="fieldset">
                  <FormLabel component="legend" style={{ marginBottom: "5px" , color: '#222222' }}>Kategori:</FormLabel>
                  <RadioGroup
                      row
                      aria-label="renk"
                      name="renk"
                      value={selectedItem?.device || ""}
                      onChange={(e) => setSelectedItem({ ...selectedItem, device: e.target.value })}
                  >
                      <FormControlLabel value="masaüstü" control={<Radio />} label="Masaüstü" />
                      <FormControlLabel value="mobil" control={<Radio />} label="Mobil" />
                  </RadioGroup>
              </FormControl>
          </div>
        ...
        <div style={{ marginTop: "20px", marginBottom: "20px", padding: "10px", border: "1px solid #ccc", borderRadius: "5px" }}>
              <FormControl component="fieldset">
                  ...
                  <RadioGroup
                      ...
                      aria-label="renk"
                      name="renk"
                      value={newItem?.device || ""}
                      onChange={(e) => setNewItem({ ...newItem, device: e.target.value })}
                  >
                      <FormControlLabel value="masaüstü" control={<Radio />} label="Masaüstü" />
                      <FormControlLabel value="mobil" control={<Radio />} label="Mobil" />
                  </RadioGroup>
